...
public class CTabRendering extends CTabFolderRenderer {
    ...
    protected Rectangle computeTrim(int part, int state, int x, int y, int width, int height) {
        ...
        case PART_BODY:
            if (state == SWT.FILL) {
                ...
                y = onBottom ? y - paddingTop - marginHeight - borderTop - (cornerSize / 4) : y - paddingTop
                        - marginHeight - tabHeight - borderTop - (cornerSize / 4);
                ...
                if (parent.getMinimized()) {
                    ...
                    // tabHeight
                    // - borderTop;
                    y = onBottom ? y - marginHeight - borderTop - (cornerSize / 4) : y - marginHeight - tabHeight
    ...
    void drawTabBody(GC gc, Rectangle bounds, int state) {
        ...
        int height = Math.max(parent.getTabHeight() + INNER_KEYLINE + OUTER_KEYLINE
                + (shadowEnabled ? BOTTOM_DROP_WIDTH : 0), bounds.height - INNER_KEYLINE - OUTER_KEYLINE - 2
    ...
    void drawSelectedTab(int itemIndex, GC gc, Rectangle bounds, int state) {
        ...
        if (itemIndex == 0 && bounds.x == -computeTrim(CTabFolderRenderer.PART_HEADER, SWT.NONE, 0, 0, 0, 0).x) {
            ...
            if (active) {
                points[index++] = shadowEnabled ? SIDE_DROP_WIDTH : 1 + INNER_KEYLINE + OUTER_KEYLINE;
                points[index++] = bottomY;
            }
            points[index++] = selectionX1 = bounds.x + 1;
            points[index++] = selectionY1 = bottomY;
        }
        ...
        gc.setClipping(0, onBottom ? bounds.y - header : bounds.y, parent.getSize().x
                - (shadowEnabled ? SIDE_DROP_WIDTH : 0 + INNER_KEYLINE + OUTER_KEYLINE), bounds.y + bounds.height);// bounds.height
        // +
        // 4);
        ...
        if (!active && !onBottom) {
            ...
            foregroundPattern = new Pattern(gc.getDevice(), 0, 0, 0, bounds.height + 1, gradientLineTop, gc.getDevice()
    ...
    void drawUnselectedTab(int itemIndex, GC gc, Rectangle bounds, int state) {
        ...
        gc.setClipping(points[0], onBottom ? bounds.y - header : bounds.y, parent.getSize().x
                - (shadowEnabled ? SIDE_DROP_WIDTH : 0 + INNER_KEYLINE + OUTER_KEYLINE), bounds.y + bounds.height);
    ...
    void drawShadow(final Display display, Rectangle bounds, GC gc) {
        ...
        // Pad the rest of the shadow
        gc.drawImage(shadowImage, SIZE, SIZE * 2, fillWidth - yFill, SIZE, yFill + SIZE, y + height - SIZE, fillWidth
                - yFill, SIZE);
    ...
    private void drawUnselectedTabBackground(GC gc, Rectangle partHeaderBounds, int state, boolean vertical,
        ...
        if (unselectedTabsColors == null) {
            boolean selected = (state & SWT.SELECTED) != 0;
            unselectedTabsColors = selected ? parentWrapper.getSelectionGradientColors() : parentWrapper
                    .getGradientColors();
            unselectedTabsPercents = selected ? parentWrapper.getSelectionGradientPercents() : parentWrapper
                    .getGradientPercents();
        }
        if (unselectedTabsColors == null) {
        ...
        public void drawBackground(GC gc, int x, int y, int width, int height, Color defaultBackground, Color[] colors,
            ...
            if (drawBackgroundMethod == null) {
                drawBackgroundMethod = getMethod("drawBackground", //$NON-NLS-1$
                        new Class<?>[] { GC.class, int[].class, int.class, int.class, int.class, int.class,
                                Color.class, Image.class, Color[].class, int[].class, boolean.class });
            }
